...
class TestBackToTopRoutes:
    ...
    def test_backtotop_css_file(self, client):
        """Test GET /css/backtotop.min.css returns the CSS file."""
        response = client.get('/css/backtotop.min.css')
        assert response.status_code == 200
        assert 'css' in response.content_type
        assert 'Cache-Control' in response.headers

    def test_backtotop_js_file(self, client):
        """Test GET /js/backtotop.min.js returns the JavaScript file."""
        response = client.get('/js/backtotop.min.js')
        assert response.status_code == 200
...
class TestBackToTopAccess:
    ...
    def test_static_routes_accessible(self, client):
        ...
        css_response = client.get('/css/backtotop.min.css')
        js_response = client.get('/js/backtotop.min.js')
